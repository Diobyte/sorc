...
local menu_elements =
{
    ...
    main_boolean        = checkbox:new(true, get_hash(my_utility.plugin_label .. "frozen_orb_main_bool_base")),
...
local function menu()
    ...
    if menu_elements.tree_tab:push("Frozen Orb")then
        menu_elements.main_boolean:render("Enable Frozen Orb", "Orb that damages enemies in its path")
...
local function logics(target)
    ...
    if not target then
        if debug_enabled then
            console.print("[FROZEN ORB DEBUG] No target")
        end
        return false;
    end;
