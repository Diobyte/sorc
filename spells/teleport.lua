...
local max_spell_range = spell_data.teleport.data.range
local targeting_type = "ranged"
local menu_elements =
{
    tree_tab            = tree_node:new(1),
    main_boolean        = checkbox:new(true, get_hash(my_utility.plugin_label .. "teleport_main_bool_base")),
    targeting_mode      = combo_box:new(0, get_hash(my_utility.plugin_label .. "teleport_targeting_mode")),
    min_target_range    = slider_float:new(1, max_spell_range - 1, 3,
        get_hash(my_utility.plugin_label .. "teleport_min_target_range")),
}

local function menu()
    if menu_elements.tree_tab:push("Teleport") then
        menu_elements.main_boolean:render("Enable Teleport", "")
...
local function logics(target)
    ...
    if cast_spell.position(spell_data.teleport.spell_id, target:get_position(), 0) then
        local current_time = get_time_since_inject();
        next_time_allowed_cast = current_time + my_utility.spell_delays.regular_cast;
        if _G.__sorc_debug__ then
            console.print("Cast Teleport - Target: " ..
