...
local menu_elements =
{
    ...
    priority_target     = checkbox:new(false, get_hash(my_utility.plugin_label .. "spark_priority_target")),
    min_mana_percent    = slider_float:new(0.1, 1.0, 0.1, get_hash(my_utility.plugin_label .. "spark_min_mana")),
...
local function logics(target)
    ...
    if mana_pct < menu_elements.min_mana_percent:get() then
        if debug_enabled then
            console.print("[SPARK DEBUG] Insufficient mana: " .. string.format("%.1f", mana_pct * 100) .. "%")
        end
        return false, 0
    end
    ...
    if not target then
        if debug_enabled then
            console.print("[SPARK DEBUG] No target found")
        end
        return false, 0
    end
